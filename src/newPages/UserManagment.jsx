...
  FormControl,
  InputLabel,
  // Removed Modal as the imported component will be used directly
  IconButton,
  Typography,
  // Note: CircularProgress, Snackbar, Alert used in AddOwnerModal/OtpContent
} from "@mui/material";
...
  // State for fetched users, loading, and error
  const [users, setUsers] = useState([]); // This will hold the actual user data from the API
  const [loadingUsers, setLoadingUsers] = useState(true);
  const [errorUsers, setErrorUsers] = useState(null);
  const navigate = useNavigate();

  // State for filters, pagination, sorting (applied to fetched users)
...
  // State for showing user account details
  const [selectedUser, setSelectedUser] = useState(null); // Will hold full details if fetched
  const [showAccount, setShowAccount] = useState(false);
  // Added loading/error states for fetching single user detail
  const [loadingUserDetail, setLoadingUserDetail] = useState(false);
  const [errorUserDetail, setErrorUserDetail] = useState(null);


  // State for Add Owner Modal visibility (This now controls the *imported* AddOwnerModal)
...
    // const [addingUser, setAddingUser] = useState(false); // <-- Removed
    // const [addUserError, setAddUserError] = useState(null); // <-- Removed
...
  // Include statuses from Account component for completeness in filters/display
...
  }
  console.log("Fetching users with token:", admin.AccessToken ? "Present" : "Missing"); // Debugging line
    try {
...
    };
    ...
    }, [adminToken, showAccount]); // Dependencies
...
    // Assuming the token is stored as AccessToken property
...
  // --- Effect to fetch users when token is available and view is list ---
  useEffect(() => {
    // This effect triggers the initial fetch and refreshes when needed
    refetchUsers();
  // Dependency array: runs when adminToken or showAccount changes, and includes refetchUsers
  // refetchUsers is stable due to useCallback, so this won't loop unnecessarily
  }, [refetchUsers]);


  // --- Handle row click to view details ---
  // Calls the top-level getUserById
  const handleRowClick = async (userInList) => {
    // Assume each user object in the list has an 'id' property
    if (userInList && userInList.id && adminToken) { // Check for token
...
  }
    } else if (!adminToken) {
    setErrorUsers("Authentication token not available to fetch user details."); // Show error in list view
    }
  else {
 console.error("User object or user ID is missing for row click.", userInList);
 setErrorUsers("Could not retrieve user ID for details."); // Show error in list view
  }
  };


  // --- Add Owner Modal Handlers ---
    // This function opens the *imported* AddOwnerModal by setting the state
  const handleOpenModal = () => {
    setOpenModal(true);
    // AddOwnerModal now manages its internal step and form state
  };
...
    // Search includes first name, last name, email, and phone number
    const matchesSearch = userName.toLowerCase().includes(searchTerm.toLowerCase()) ||
 userLastName.toLowerCase().includes(searchTerm.toLowerCase()) ||
 userEmail.toLowerCase().includes(searchTerm.toLowerCase()) ||
   userPhone.includes(searchTerm); // Simple phone search

    // Note: Your matchesRole filter still compares userEmail to filters.role,
...
    setErrorUserDetail(null); // Clear any detail error
    // The useEffect watching showAccount will trigger fetchUsers here
  }}
    >
  User Management
    </button>,
...
    ) : errorUserDetail ? (
   <div className="text-center py-8 text-red-600">Error: {errorUserDetail}</div>
...
    className={`bg-[#00173C] cursor-pointer w-28 justify-center h-fit text-xs text-white flex items-center shadow-lg px-4 py-3 rounded-4xl mr-4 mx-2 ${!adminToken ? 'opacity-50 cursor-not-allowed' : ''}`}
...
    ) : errorUsers ? (
  <div className="text-center py-8 text-red-600">Error: {errorUsers}</div>
    ) : (
...
  {user.first_name || 'N/A'} {/* Display using new key */}
    </td>
    <td className="px-6 py-4 text-sm text-gray-700">
  {user.registrationDate || 'N/A'}
    </td>
    <td className="px-6 py-4 text-sm text-gray-700">
  {user.last_name || 'N/A'} {/* Display using new key */}
    </td>
    <td className="px-6 py-4 text-sm text-gray-700">
  {user.email || 'N/A'}
    </td>
...
  statusColors[user.status] || 'text-gray-700 bg-gray-100'
    }`}
  >
    {user.status || 'N/A'}
  </span>
